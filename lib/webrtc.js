...
export class PeerManager {
    ...
    createPeer(peerId, initiator = false) {
        ...
        console.log(`🛠️ Creating Peer: ${peerId} (Initiator: ${initiator})`);
        ...
        peer.on('signal', (data) => {
            console.log(`📤 Emitting signal to ${peerId} (type: ${data.type})`);
    ...
    handleSignal(peerId, signal) {
        ...
        // Deduplicate signals: If this exact signal was just processed, ignore it.
        const signalStr = JSON.stringify(signal);
        if (peer._lastSignal === signalStr) {
            console.log(`🔄 Ignoring exact duplicate signal from ${peerId}`);
            return;
        }
        peer._lastSignal = signalStr;
    ...
    updateLocalStream(newStream) {
        console.log(`🔄 PeerManager: Updating local stream for ${this.peers.size} peers`);
        ...
        this.peers.forEach((peer, peerId) => {
            try {
                ...
                if (oldStream) {
                    console.log(`  - Removing old stream for ${peerId}`);
                    peer.removeStream(oldStream);
                }
                // Add new tracks
                if (newStream) {
                    console.log(`  - Adding new stream for ${peerId}`);
                    peer.addStream(newStream);
                }
            } catch (err) {
                console.error(`Error updating stream for ${peerId}:`, err);
            }
        });
    }

    // Stop and remove local stream from all peers
    stopLocalStream() {
        ...
        this.peers.forEach((peer) => {
            try {
                ...
                console.warn('Failed to remove stream on stop:', e);
            }
        });
