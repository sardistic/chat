...
class IRCBridge {
    constructor(socket, userConfig, options = {}) {
        ...
        this.config = {
            ...
            tls: userConfig.tls !== undefined ? userConfig.tls : true // Default to TLS true for Ergo
        };
    ...
    connect() {
        ...
        this.client.connect({
            ...
            tls: this.config.tls,
            // Self-signed certs might be needed for some testnets, but Ergo has valid certs usually.
        ...
        this.client.on('privmsg', (event) => {
            if (this.shouldIgnoreSender && this.shouldIgnoreSender(event.nick)) {
                return;
            }
            ...
            if (event.target.startsWith('#')) {
                ...
                // Broadcast logic:
                // 1. If Bot: Broadcast to room (HistoryBot responsibility)
                // 2. If User: Only broadcast if NOT the main public channel (HistoryBot handles that)
                const isPublicChannel = event.target.toLowerCase() === '#camsrooms';
                const shouldBroadcast = this.config.isBot || !isPublicChannel;

                if (shouldBroadcast) {
                    console.log(`[IRC-TRACE] 🔊 Broadcasting from ${this.config.nick} (isBot: ${this.config.isBot}) Source: ${event.nick} Target: ${event.target}`);
                    if (this.io) {
                    ...
                    console.log(`[IRC-TRACE] 🔇 Suppressed from ${this.config.nick} (isBot: ${this.config.isBot}) Source: ${event.nick}`);
                }
