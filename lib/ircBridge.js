...
class IRCBridge {
    ...
    connect() {
        console.log(`[IRC] Connecting ${this.config.nick} to ${this.config.host}:${this.config.port} (TLS: ${this.config.tls})`);
        ...
        this.client.on('privmsg', (event) => {
            ...
            if (event.target.startsWith('#')) {
                ...
                // Broadcast to ALL clients via io (if available)
                if (this.io) {
                    this.io.to('default-room').emit('chat-message', message);
                }
                // Or emit to ownership socket only
                else if (this.socket) {
